...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    if noisyDistance == None:
        ...
        for p in self.legalPositions:
            ...
            # P(noisyDist|trueDist) = emissionModel[trueDist] = P(currentEvidence|currentState)
            # Equation is: B(X_t+1) = P(e_t+1|X_t+1)B'(X_t+1)
  ...
  def elapseTime(self, gameState):
    ...
    for pos in self.legalPositions:
        ...
        for newPos, prob in newPosDist.items():
            # newpostDist[p] = Pr(x_t|x_t-1)
            # Equation: P(x_t|e_1:t-1)=sum(P(x_t-1|e_1:t-1)*P(x_t|x_t-1))
...
class ParticleFilter(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    previousBelief = self.getBeliefDistribution()
    
    if noisyDistance == None:
        # Case: a ghost is captured
        for i in range(self.numParticles):
            self.particle[i] = self.getJailPosition()
    else:
        for pos in self.legalPositions:
            distance = util.manhattanDistance(pos, pacmanPosition)
            newBelief[pos] = newBelief[pos] + emissionModel[distance] * previousBelief[pos]
    
    if newBelief.totalCount() == 0:
        # All particles receive 0 weight
        self.initializeUniformly(gameState)
    else:
        sampleList = [util.sample(newBelief) for i in range(self.numParticles)]
  ...
  def elapseTime(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    # x' = sample(P(X'|x))
    # Reference: https://github.com/yuxinzhu/tracking/blob/master
    temp = []
  ...
  def initializeParticles(self):
    ...
    for i in range(self.numParticles):
        ...
        for j in range(self.numGhosts):
            ghostPosition.append(random.choice(self.legalPositions))
        self.particles.append(tuple(ghostPosition))
  ...
  def observeState(self, gameState):
    ...
    for ghostIndex in range(self.numGhosts):
        if noisyDistances[ghostIndex]==None:
            for i in range(self.numParticles):
                copyOfParticle = list(self.particles[i])
                copyOfParticle[ghostIndex] = self.getJailPosition(ghostIndex)
                self.particles[i] = tuple(copyOfParticle)
    ...
    for particle in self.particles:
        ...
        for ghostIndex in range(self.numGhosts):
            if noisyDistances[ghostIndex] != None:
                ...
                prob = emissionModels[ghostIndex][distance] * prob
        tempList[particle] = tempList[particle] + prob
    
    if tempList.totalCount()==0:
        self.initializeParticles()
    else:
	    for i in range(self.numParticles):
	        newParticles.append(util.sample(tempList))
	    self.particles = newParticles
