...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    livingGhostPositionDistributions = [beliefs for i,beliefs
                                        ...
                                        if livingGhosts[i+1]]
    "*** YOUR CODE HERE ***"
    # Step1: find the most likely position of ghost (in prob)
    mostLikelyPos = None
    maxProb = 0
    for ghostBelief in livingGhostPositionDistributions:
        currentPos = ghostBelief.argMax()
        currentProb = ghostBelief[currentPos]
        
        if currentProb > maxProb:
            maxProb = currentProb
            mostLikelyPos = currentPos

    # Step 2: choose an action that minimizes the distance to closest ghost
    bestAction = None
    minDistance = float('inf')
    for action in legal:
        successorPosition = Actions.getSuccessor(pacmanPosition, action)
        distance = self.distancer.getDistance(successorPosition, mostLikelyPos)
